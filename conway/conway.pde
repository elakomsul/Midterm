...
void setup()
{
  size(500, 500); // make the screen square
...
void draw()
{
  ...
  if (mousePressed)
  {
    int mx = constrain(int(mouseX/15.0), 0, width-15);
    int my = constrain(int(mouseY/15.0), 0, height-15);
  ...
  for (i=0; i<h; i++) // these pixels are the rows
  {
    for (j=0; j<w; j++) // thise pixels are the columns
    {
      ...
      //
      if (center==1 && (sum==2 || sum==3 || sum==1)) // alive... stay alive
      { 
        c[1-which].pixels[i*w + j] = color(25, 190, 200);
